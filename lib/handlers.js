...
// Users - delete
// Required - phone
// TODO clean up any other data files assigned to user
...
handlers._checks.get = (data, callback) => {
  ...
  if (eitherId.isRight()) {
    // look up the token
    const readCallback = (checkData) => {
      // Verify that the given token is valid and belongs to the user
      const eitherToken = validators.isStringWithLength(20)('token', data.headers.token)
      if (eitherToken.isRight()) {
        const verifyCallback = (tokenIsValid) => (tokenIsValid
            ? callback(200, checkData)
            : callback(403, {
                Error: 'User is not authorized according to token in headers',
              }))

        handlers._tokens.verifyTokenP(eitherToken.right().token, checkData.userPhone).then(verifyCallback)
      } else {
        callback(404, { Error: 'Token is missing' })
      }
    }
    _data.readP('checks', eitherId.right().id).then(readCallback, (err) => callback(404, { Error: err }))
  } else {
    callback(400, {
