...
workers.alertUserToStatusChange = (checkData) => {
  ...
  helpers.sendTwilioSms(checkData.phone, msg, (error) => {
    if (!error) {
      console.log('Success: User was alerted to a status change in ther check')
    } else {
      console.log('Error: Could not send sms alert to user who had a state change to their check')
    }
  })
}
...
workers.processCheckOutcome = (checkData, checkOutcome) => {
  ...
  _data
    ...
    .catch((e) => console.log(`Error trying to update checkdata: ${e.message}`))
}
...
workers.validateCheckData = (checkData) => {
  // console.log('validateCheckData', checkData)
  ...
  if (errorMessages.length > 0) {
    return console.log(`CheckData is not valid: ${errorMessages.join(';')}`)
  }
...
workers._readCheckP = (checkFile) => _data
    ...
    .then(workers.validateCheckData, (e) => console.log(`Error: could not find the check to process - ${e.message}`))
...
workers.gatherAllChecks = () => {
  // Get all the checks
  _data.listP('checks').then(workers._readChecks, () => console.log("Error reading one of the check's data"))
}
      ...
      list.forEach((logName) => {
        ...
        _logs
          ...
          .then(() => {
            ...
            _logs
              .truncateP(logId)
              .then(() => console.log(`Success truncating the file ${logId}`))
              .catch((truncateError) => console.log(`Error truncating the file ${logId}; ${truncateError.message}`))
          })
          .catch((compressError) => console.log(`Could not compress the file ${logId}; ${compressError.message}`))
      })
    })
    .catch((error) => {
      console.log(`Error: could not find any logs to rotate: ${error.message}`)
    })
}
...
workers.log = (checkData, checkOutcome, state, alertWarranted, timeOfCheck) => {
  ...
  _logs
    ...
    .then(() => console.log(`Logging for id = ${logFileName} succeded`))
    .catch((error) => console.log(`Logging for id = ${logFileName} failed: ${error.message}`))
}
